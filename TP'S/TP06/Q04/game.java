// Matheus Gouvêa Ramalho - TP06 - Q04 - Fila Encadeada de Games
...
public class game {
        ...
        while (true) {
            ...
            try {
                ...
                if (gameEncontrado != null) {
                    filaDeRegistros.Inserir(gameEncontrado);
                }
            } catch (Exception e) {
    ...
    for (int i = 0; i < numComandos; i++) {
        ...
        try {
            switch (comando) {
                case "I": 
                    int idInserir = Integer.parseInt(partes[1]);
                    game gameI = buscarGame(idInserir, jogos, qtdJogos);
                    if (gameI != null) {
                        filaDeRegistros.Inserir(gameI); // Empilha
                    }
                    break;

                case "R": 
                    gameRemovido = filaDeRegistros.Remover(); // Desempilha
                    break;
            }

            // Saída para registros removidos: (R) 
