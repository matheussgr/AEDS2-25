...
// Selection Sort baseado no campo 'name'
void selectionSort(Game** array, int n, long long* comparacoes, long long* movimentacoes) {
    for (int i = 0; i < (n - 1); i++) {
        int menor = i;
        for (int j = (i + 1); j < n; j++) {
            (*comparacoes)++;
...
int main(void) {
    ...
    Game** jogosParaOrdenar = (Game**)malloc(sizeof(Game*) * 100);
    ...
    if (countParaOrdenar > 0) {
        inicio = clock();
        selectionSort(jogosParaOrdenar, countParaOrdenar, &comparacoes, &movimentacoes);
        fim = clock();
